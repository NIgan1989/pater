...
class _SearchContentState extends State<SearchContent>
  ...
  void _searchLocationOnMap(String query) {
    // Полноценная реализация поиска местоположения по строке с использованием геокодинга
    if (query.isEmpty) return;

    // Примечание: в реальном приложении здесь использовался бы плагин geocoding
    // Для примера реализуем базовую логику с несколькими предопределенными городами
    Map<String, LatLng> cities = {
      'алматы': const LatLng(43.238949, 76.889709),
      'астана': const LatLng(51.160522, 71.470356),
      'шымкент': const LatLng(42.315514, 69.586907),
      'москва': const LatLng(55.755826, 37.617300),
      'санкт-петербург': const LatLng(59.939099, 30.315877),
    };

    // Приводим запрос к нижнему регистру для поиска
    final lowerCaseQuery = query.toLowerCase();

    // Ищем совпадение среди предопределенных городов
    for (final city in cities.keys) {
      if (city.contains(lowerCaseQuery)) {
        // При нахождении города обновляем состояние для центрирования карты
        if (mounted) {
          setState(() {
            // Здесь бы обновляли центр карты на найденный город
            // Для реализации в полном приложении требуется доступ к контроллеру карты
            debugPrint('Найден город: $city на координатах: ${cities[city]}');
          });
        }
        return;
      }
    ...
    // Если город не найден, можно показать сообщение пользователю
    debugPrint('Город не найден по запросу: $query');
  }
