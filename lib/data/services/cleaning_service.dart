...
class CleaningService {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      debugPrint(
        'Сервис уборки инициализирован. Заявок в базе: ${snapshot.docs.length}',
      );
  ...
  Future<CleaningRequest> createCleaningRequest({
    ...
    try {
      ...
      if (user.id != ownerId) {
        throw Exception(
          'Вы не можете создать заявку для объекта, который вам не принадлежит',
        );
      }
  ...
  Future<CleaningRequest?> getCleaningRequestById(String requestId) async {
    try {
      final documentSnapshot =
          await _firestore.collection('cleaning_requests').doc(requestId).get();
  ...
  Future<List<CleaningRequest>> getCleaningRequestsByOwnerId(
    String ownerId,
  ) async {
    try {
      final querySnapshot =
  ...
  Future<List<CleaningRequest>> getCleanerActiveRequests(
    String cleanerId,
  ) async {
    try {
      final querySnapshot =
  ...
  Future<List<CleaningRequest>> getCleanerCompletedRequests(
    String cleanerId,
  ) async {
    try {
      final querySnapshot =
          await _firestore
              ...
              .where(
                'status',
                isEqualTo:
                    CleaningRequestStatus.completed.toString().split('.').last,
              )
              .get();
  ...
  Future<void> acceptCleaningRequest(String requestId, String cleanerId) async {
    try {
      ...
      await _notificationService.sendCleaningApprovedNotification(
        requestId: requestId,
        propertyTitle:
            (await _propertyService.getPropertyById(
              request.propertyId,
            ))?.title ??
            'Объект',
        cleanerId: cleanerId,
  ...
  Future<void> updateCleaningRequestStatus(
    String requestId,
    CleaningRequestStatus newStatus,
  ) async {
    try {
      // Получаем текущую заявку
      final request = await getCleaningRequestById(requestId);
  ...
  Future<void> _notifyCleanersAboutNewRequest(
    CleaningRequest request,
    Property property,
  ) async {
    try {
      // Получаем список клинеров
      final cleanersSnapshot =
          await _firestore
              .collection('users')
              .where(
                'role',
  ...
  Future<List<CleaningRequest>> getCleaningRequestsByCleanerId(
    String cleanerId,
  ) async {
    try {
      final querySnapshot =
  ...
  Future<CleaningRequest?> getActiveCleaningRequestForProperty(
    String propertyId,
  ) async {
    try {
      final querySnapshot =
  ...
  Future<void> acceptOffer(String requestId, String offerId) async {
    try {
      ...
      if (request.status != CleaningRequestStatus.withOffers &&
        ...
        throw Exception(
          'Заявка не может принять предложение в текущем статусе',
        );
      }
